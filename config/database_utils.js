...
const dbConfig = require('./db_config'); // Assuming it's in config/db_config.js
const { logStep, logSuccess, logFailure, logInfo } = require('../test/test_utils');
...
async function updateTourStartTime(tourId, newStartTime) {
    ...
    try {
        ...
        logStep(`DB: Connected to database.`);
        ...
        if (rows.affectedRows > 0) {
            logStep(`DB: Tour ${tourId} start date updated to ${formattedDate} and time to ${formattedTime}.`);
            return true;
        } else {
            logStep(`DB: No tour found with ID ${tourId} or time/date were already set.`);
            return false;
        }
    } catch (error) {
        logStep('DB: Error updating tour start time:', error);
        throw error; // Re-throw to be caught by the main test function
    } finally {
        if (connection) {
            await connection.end();
            logStep(`DB: Disconnected from database.`);
        }
    }
}
